...
function getSimplexNoise() {
  return window.SimplexNoise;
}
...
export function getBlockType(x, y, z) {
  ...
  return chunkManager.getBlockType(
    chunkX,
    chunkY,
    chunkZ,
    localX,
...
function calculateTerrainHeight(x, z, worldConfig, seed) {
  const SimplexNoiseClass = getSimplexNoise();

  const noise = new SimplexNoiseClass(seed);
  ...
  const surfaceHeight = Math.floor(
    worldConfig.BASE_GROUND_LEVEL +
      normalizedNoise * worldConfig.TERRAIN_HEIGHT_RANGE
  );

  return Math.min(surfaceHeight, worldConfig.MAX_HEIGHT);
}

function findSafeSpawnHeight(x, z, worldConfig, seed) {
  const terrainHeight = calculateTerrainHeight(x, z, worldConfig, seed);

  if (terrainHeight < worldConfig.WATER_LEVEL) {
    return worldConfig.WATER_LEVEL + 1.7;
  }

  return terrainHeight + 1.7;
}
...
export function spawn(x, z) {
  ...
  const spawnX = worldSize / 2 + 0.5;
  const spawnZ = worldSize / 2 + 0.5;
  const spawnY = findSafeSpawnHeight(spawnX, spawnZ, worldConfig, seed);
