...
export const PHYSICS_CONSTANTS = {
  ...
  GRAVITY: 32.0, // blocks/second² (0.08 * 20² = 32)
  JUMP_SPEED: 8.4, // blocks/second (0.42 * 20 = 8.4)
  TERMINAL_VELOCITY: -78.4, // blocks/second (-3.92 * 20 = -78.4)
  ...
  WALK_SPEED: 4.3, // blocks/second (0.215 * 20 = 4.3)
  SPRINT_SPEED: 5.6, // blocks/second (0.28 * 20 = 5.6)
  SNEAK_SPEED: 1.3, // blocks/second (0.065 * 20 = 1.3)
  FLY_SPEED: 10.0, // blocks/second (faster for creative mode)
};
...
export function applyVerticalPhysics(entity, controls, scene, deltaTime) {
  ...
  if (controls.jump && isOnGround) {
    ...
    let clearSpace = 0;
    const maxJumpHeight = 1.25; // Minecraft-like jump height
    ...
    if (clearSpace < 0.2) {
      ...
      // Use kinematic equation: v = sqrt(2 * g * h)
  ...
  // Apply gravity: acceleration reduces velocity over time
  physicsState.yVelocity = Math.max(
    physicsState.yVelocity - PHYSICS_CONSTANTS.GRAVITY * deltaTime,
    PHYSICS_CONSTANTS.TERMINAL_VELOCITY
  );

  // Apply vertical movement: position changes based on velocity and time
  ...
  if (finalCollision.collides && entity.collisionsEnabled) {
    ...
    if (finalCollision.collisionBlock) {
      ...
      const awayVector = new THREE.Vector3(
        ...
        .normalize()
        .multiplyScalar(0.1); // Small nudge
