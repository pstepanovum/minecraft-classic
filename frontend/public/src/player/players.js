    ...
    const toggleFly = (event) => {
      if (player) {
        ...
        if (button) {
          ...
          if (icon) {
            icon.className = player.isFlying
              ? "fas fa-landing"
              : "fas fa-plane";
          }
        }
  ...
  if (collisionsButton) {
    const toggleCollisions = (event) => {
      if (player) {
        ...
        collisionsButton.classList.toggle(
          "highlight",
          !player.collisionsEnabled
        );
        console.log(
          `Collisions ${player.collisionsEnabled ? "enabled" : "disabled"}`
        );
      }
      event.preventDefault();
    };
...
export function createPlayer(scene, playerData, textureAtlas, isLocalPlayer) {
  ...
  player.collisionsEnabled =
    playerData.collisionsEnabled !== undefined
      ? playerData.collisionsEnabled
      : true;

  // Get spawn position
  const spawnPosition = GameState.spawn(
...
function createPlayerParts(material) {
  ...
  function createLimb(uvMap, position) {
    ...
    limbGeometry.setAttribute(
      "uv",
      new THREE.Float32BufferAttribute(limbUvs, 2)
    );
  ...
  const limbs = [
    {
      ...
      position: new THREE.Vector3(
        -BODY_WIDTH / 2 - LIMB_WIDTH / 2,
        PLAYER_HEIGHT / 2 - LIMB_HEIGHT / 2,
        0
      ),
    },
    {
    ...
    },
    {
      ...
      position: new THREE.Vector3(
        -BODY_WIDTH / 4,
        PLAYER_HEIGHT / 2 - BODY_HEIGHT - LIMB_HEIGHT / 2,
        0
      ),
    },
    {
...
export function addPlayerControls(player, camera, scene, canvas) {
  ...
  function updatePlayerMovement(deltaTime) {
    ...
    // Calculate movement vector WITH delta time
    moveVector
      .set(
    ...
    if (!player.isFlying) {
      Physics.applyVerticalPhysics(player, controls, scene, deltaTime);
    }

    // Update camera position
    camera.position
      .copy(player.position)
      .add(new THREE.Vector3(0, PLAYER_HEIGHT * 0.6, 0));

    // Network code and event publishing
    if (
      GameState.isOnline &&
      GameState.socket &&
      player.userData.id === GameState.socket.id
    ) {
...
export function handlePlayerMove(playerData) {
  ...
  // Update player state
  otherPlayer.isFlying =
    playerData.isFlying !== undefined
      ? playerData.isFlying
      : otherPlayer.isFlying;
  otherPlayer.collisionsEnabled =
    playerData.collisionsEnabled !== undefined
      ? playerData.collisionsEnabled
      : otherPlayer.collisionsEnabled;
