...
function handleWorldInfo(data) {
  ...
  if (data.modifications && data.modifications.length > 0) {
    console.log(
      `Received ${data.modifications.length} modifications from server`
    );
...
async function init() {
  ...
  try {
    ...
    const serverAvailable = await GameState.checkServerStatus(
      CONFIG.SERVER_URL
    );
  ...
  GameState.renderer.setAnimationLoop(() => {
    ...
    if (GameState.playerControls && GameState.player) {
      ...
      if (
        !GameState.isOnline ||
        (GameState.isOnline &&
          GameState.player.userData.id === GameState.socket?.id)
      ) {
...
function setupSocketConnection() {
  ...
  socket.on("connect", async () => {
    ...
    socket.emit("playerJoin", {
      ...
      collisionsEnabled: GameState.player
        ? GameState.player.collisionsEnabled
        : true,
    });
